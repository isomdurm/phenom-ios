...
//        }
    ...
    fileprivate var tempFilePath: URL = {
        let tempPath = URL(fileURLWithPath: NSTemporaryDirectory()).URLByAppendingPathComponent("tempMovie").URLByAppendingPathExtension("mp4").absoluteString
        if FileManager.defaultManager().fileExistsAtPath(tempPath) {
            do {
                try FileManager.defaultManager().removeItemAtPath(tempPath)
            } catch { }
        }
...
//                    })
                    ...
                    _executeVideoCompletitionWithURL(outputFileURL, error: error as! NSError)
                }
            }
        }
    }
